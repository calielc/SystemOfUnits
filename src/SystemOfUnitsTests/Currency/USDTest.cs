...
namespace SystemOfUnitsTests.Currency {
    using USD = SystemOfUnits.Currency.USD;

    [Parallelizable(ParallelScope.Default)]
    public sealed partial class USDTest {
        [Test]
        public void Should_have_symbol_on_class() {
            Assert.AreEqual("US$", USD.Symbol);
        }
        ...
        public void Should_have_a_symbol(decimal value) {
            ICurrency actual = new USD(value);

            Assert.AreEqual(USD.Symbol, actual.Symbol);
        }
        ...
        public void Should_clone(decimal value) {
            ...
            ICloneable cloneable = new USD(value);
